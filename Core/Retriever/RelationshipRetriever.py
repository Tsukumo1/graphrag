...
class RelationshipRetriever(BaseRetriever):
    ...
    async def _find_relevant_relations_by_entity_agent(self, query: str, entity: str, pre_relations_name=None,
        ...
        try:
            ...
            reverse_relations_dict = defaultdict(list)
            for index, edge in enumerate(edges):
                src, tar = edge[0], edge[1]
                for rel in relations_name[index]:
                    relations_dict[(src, rel)].append(tar)
                    # 构建反向索引，便于 tail 方向获取另一端节点
                    reverse_relations_dict[(tar, rel)].append(src)
            ...
            # 限制关系数量以避免token超限
            max_relations = 2000  
            if len(total_relations) > max_relations:
                print(f"Too many relations ({len(total_relations)}), limiting to {max_relations}")
                total_relations = total_relations[:max_relations]

            head_relations = set(head_relations)

            # 基于邻接信息构造三元组候选，以 [node1.relation.node2] 文本格式供打分
            candidate_triples_texts = []
            # head 方向：entity 为 src
            for rel in head_relations:
                targets = relations_dict.get((entity, rel), [])
                for tgt in targets[:10]:  # 每个关系最多取若干示例，避免过长
                    candidate_triples_texts.append(f"[{entity}.{rel}.{tgt}]")
            # tail 方向：entity 为 tar（需要用反向索引）
            tail_only = set(total_relations) - head_relations
            for rel in tail_only:
                sources = reverse_relations_dict.get((entity, rel), [])
                for src in sources[:10]:
                    candidate_triples_texts.append(f"[{src}.{rel}.{entity}]")

            # 限制总候选数量，避免 prompt 过长
            max_triples = 100
            if len(candidate_triples_texts) > max_triples:
                candidate_triples_texts = candidate_triples_texts[:max_triples]

            # 若未能构造任何三元组，则退回到仅用关系名
            relations_or_triples_str = '; '.join(candidate_triples_texts) if candidate_triples_texts else '; '.join(total_relations)

            # agent 提示（复用模板，但传入三元组文本）
            prompt = extract_relation_prompt % (str(width), str(width), str(width), query, entity, relations_or_triples_str)
            ...
            # clean
            import re
            pattern = r"{\s*(?P<item>[^()]+)\s+\(Score:\s+(?P<score>[0-9.]+)\)}"
            relations = []
            for match in re.finditer(pattern, result):
                ...
                # 解析三元组文本 [node1.rel.node2]，否则回退为 relation 名
                triple_src = triple_rel = triple_tgt = None
                cleaned = item.strip().strip('[]')
                parts = [p.strip() for p in cleaned.split('.')]
                if len(parts) == 3:
                    triple_src, triple_rel, triple_tgt = parts
                    is_head = (triple_src == entity)
                    connected_nodes = [triple_tgt] if is_head else [triple_src]
                    relations.append({
                        "entity": entity,
                        "relation": triple_rel,
                        "score": score,
                        ...
                        "triples": [(triple_src, triple_rel, triple_tgt)],
                    })
                else:
                    relation = item
                    is_head = relation in head_relations
                    if is_head:
                        connected_nodes = relations_dict.get((entity, relation), [])
                        triples = [(entity, relation, tgt) for tgt in connected_nodes]
                    else:
                        connected_nodes = reverse_relations_dict.get((entity, relation), [])
                        triples = [(src, relation, entity) for src in connected_nodes]
                    relations.append({
                        "entity": entity,
                        "relation": relation,
                        "score": score,
