...
class LeidenCommunity(BaseCommunity):
    ...
    async def _form_single_community_report(self, er_graph, community,
        ...
        print("_form_single_community_report")
        response = await self.llm.aask(prompt, format = "json", timeout=600)
    ...
    async def _pack_single_community_describe(
            self, er_graph: BaseGraph, community: LeidenInfo, max_token_size: int = 2000, # int=12000
        ...
        # nodes_in_order = nodes_in_order[:100]
        # edges_in_order = edges_in_order[:100]
        ...
        if len(nodes_may_truncate_list_data)>10:
            nodes_may_truncate_list_data = nodes_may_truncate_list_data[:10]
        if len(edges_may_truncate_list_data)>50:
            edges_may_truncate_list_data = edges_may_truncate_list_data[:50]
        ...
        # print(f"Nodes: {len(nodes_list_data)} -> {len(nodes_may_truncate_list_data)}; "
        #         f"Edges: {len(edges_list_data)} -> {len(edges_may_truncate_list_data)}; ")
        return f"""-----Reports-----
            ```csv
            {report_describe}
            ```
