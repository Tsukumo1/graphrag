extract_relation_prompt = """Please retrieve %s relations that contribute to the question and rate their contribution on a scale from 0 to 1 (the sum of the scores of %s relations is 1). If the number of given relations is less than %s, please return all these relations with the sum of their scores equals to 1.

Q: Who wanted to cooperate with the citizens of Brazil in 2005?
Topic Entity: citizen brazil
Relations: [head of government brazil.express intent to cooperate on 2005 08 30.citizen brazil]; [cristina fernández de kirchner.express intent to cooperate on 2011 07 28.citizen brazil]; [criminal brazil.use unconventional violence on 2005 01 02.citizen brazil]; [ministry brazil.investigate on 2011 06 17.citizen brazil];

A:
1. {[head of government brazil.express intent to cooperate on 2005 08 30.citizen brazil] (Score: 1.0)}: This relation is directly relevant because it shows that the head of government of Brazil expressed the intent to cooperate with its citizens in 2005, exactly matching the question.
2. {[cristina fernández de kirchner.express intent to cooperate on 2011 07 28.citizen brazil] (Score: 0.5)}: The action "express intent to cooperate" is correct, but the time is wrong (2011 instead of 2005).
3. {[criminal brazil.use unconventional violence on 2005 01 02.citizen brazil] (Score: 0.6)}: The time (2005) is correct, but the action is wrong ("use unconventional violence" instead of "cooperate").
4. {[ministry brazil.investigate on 2011 06 17.citizen brazil] (Score: 0.2)}: This relation is irrelevant because it describes an investigation in 2011, not cooperation in 2005.

Above is example, now please output the relations in terms of , such as "{[relation] (Score: 0.2)}".
Q: %s
Topic Entity: %s
Relations: %s"""

score_entity_candidates_prompt = """Please score the entities' contribution to the question on a scale from 0 to 1 (the sum of the scores of all entities is 1). If the number of given entities is not enough, please return all these entities with the sum of their scores equals to 1.
...
Instructions:
- Always produce exactly 10 candidate answers, even if uncertain.
- Do NOT say you don't know or that information is insufficient.
- At the very beginning of the output, include exactly one {yes} or {no} to indicate whether the model believes the current information is sufficient to obtain an accurate answer.
- Output strictly in the following format (English only):
  {yes} or {no},
