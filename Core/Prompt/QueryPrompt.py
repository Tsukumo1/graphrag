...
DALK_RERANK_PROMPT = """
    ...
    ##Question: {question}
    \n\n
    Please rerank the knowledge graph and output at most 15 important and relevant triples for solving the given question. Output the reranked knowledge in the following format:
...
DALK_QUERY_PROMPT = """
    ...
    \n\n
    {context}
    Based ONLY on the context above (triples and their natural-language conversions), output exactly 10 most plausible answers in English.
    - Do not say you don't know; always produce 10 candidates.
    - Each answer must be a short phrase that directly answers the question.
    - Strict output format (no extra text, headers, or explanations):
