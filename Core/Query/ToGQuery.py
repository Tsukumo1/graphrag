...
class ToGQuery(BaseQuery):
    ...
    async def _retrieve_relevant_contexts(self, query: str, mode: str) -> str:
        if mode == "retrieve":
            ...
            print("pre_relations_name write in: ", self.pre_relations_name)
    ...
    async def _half_stop(self, query, depth, pcst_start, model):
        ...
        for depth in range(1, self.config.depth + 1):
            ...
            if context:
                ...
                if is_stop:
                    ...
                    print(f"Average retrival time={(re_time):.4f}s ")
                    ...
                    if all_candidates_are_finish:
                        ...
                        response = await self._half_stop(query, depth, pcst_start, model)
                        print(f"Time after llm={(time.time()-pcst_start):.4f}s ")
                        break
                    else:
