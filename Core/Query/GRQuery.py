...
class GRQuery(BaseQuery):
    ...
    async def retrieval_via_pcst(
        ...
        if topk > 0:
            ...
            # 打印高分nodes
            print(f"\n=== 高分Nodes (top-{topk}) ===")
            for i, entity in enumerate(retrieve_entity[:topk]):
                score = topk - i
                print(f"Node {i+1}: {entity['entity_name']} (score: {score})")
        else:
            n_prizes = torch.zeros(len(self.nodes))
        ...
        if topk_e > 0:
            ...
            # 打印高分relationships
            print(f"\n=== 高分Relationships (top-{topk_e}) ===")
            for i, (rel, score) in enumerate(zip(retrieve_relations[:topk_e], topk_e_values[:topk_e])):
                print(f"Relation {i+1}: {rel['src_id']} --[{rel['relation_name']}]--> {rel['tgt_id']} (score: {score:.4f})")
            ...
            for k in range(min(topk_e, len(topk_e_values))):
        ...
        # 打印最终选中的nodes和edges
        print(f"\n=== PCST算法选中的Nodes ({len(selected_nodes)}个) ===")
        for i, node_id in enumerate(selected_nodes):
            node_name = self.nodes.iloc[node_id]['node_attr']
            print(f"Selected Node {i+1}: {node_name} (id: {node_id})")
        
        print(f"\n=== PCST算法选中的Edges ({len(selected_edges)}个) ===")
        for i, edge_id in enumerate(selected_edges):
            edge = self.edges.iloc[edge_id]
            src_name = self.nodes.iloc[edge['src']]['node_attr']
            dst_name = self.nodes.iloc[edge['dst']]['node_attr']
            print(f"Selected Edge {i+1}: {src_name} --[{edge['edge_attr']}]--> {dst_name}")
