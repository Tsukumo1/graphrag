...
class RKGraph(BaseGraph):
    ...
    @staticmethod
    async def _build_graph_from_tuples_withtime(entities, triples, chunk_key):
        """
        Build a graph structure from entities and triples for RKGraph.
        This method provides compatibility with the ERGraph interface.

        Args:
            entities (List[str]): A list of entity strings.
            triples (List[Tuple[str, str, str, str]]): A list of triples, where each triple contains four strings (source entity, relation, target entity, description).
        ...
        for _entity in entities:
            ...
            entity = Entity(
                entity_name=entity_name, 
                entity_type="",  # RKGraph doesn't require entity type
                description="",  # Can be empty for RKGraph
                source_id=chunk_key
            )
        ...
        for triple in triples:
            if isinstance(triple[0], list): 
                triple = triple[0]
            if len(triple) != 4:
                logger.warning(f"triples length is not 4, triple is: {triple}, len is {len(triple)}, so skip it")
                continue
            
            src_entity = clean_str(triple[0])
            tgt_entity = clean_str(triple[2])
            relation_name = clean_str(triple[1])
            time = clean_str(triple[3])
            ...
            if isinstance(src_entity, str) and isinstance(tgt_entity, str) and isinstance(relation_name, str):
                relationship = Relationship(
                    ...
                    weight=1.0, 
                    source_id=chunk_key,
                    relation_name=f"{relation_name} on {time}",
                    keywords=time  # RKGraph can have empty keywords
                )
                maybe_edges[(relationship.src_id, relationship.tgt_id)].append(relationship)
